...
def main(_):
    ...
    for title_id in range(len(titles)):
        ...
        for i in range(1,num_frames):
            ...
            cur_ori_img_array = np.array(cur_ori_img)
            # cv2.rectangle(cur_ori_img_array, (int(detection_box[0, 1]), int(detection_box[0, 0])),
            #               (int(detection_box[0, 3]), int(detection_box[0, 2])), (0, 255, 255))
            #
            # for t in range(1,20):
            #     cv2.rectangle(cur_ori_img_array,(int(detection_box[t,1]), int(detection_box[t,0])),(int(detection_box[t,3]), int(detection_box[t,2])), (0,0,255))
            # cv2.imshow("haha",cur_ori_img_array[:, :, -1::-1])
            # cv2.waitKey(0)

            overlap = []
            for t in range(20):
                x1 = max(last_gt[1], detection_box[t,1])
                y1 = max(last_gt[0],detection_box[t,0])
                x2 = min(last_gt[3],detection_box[t,3])
                y2 = min(last_gt[2],detection_box[t,2])
                tmp = (x2-x1)*(y2-y1)
                if tmp > 0:
                    overlap.append(tmp / float((last_gt[2]-last_gt[0]) * (last_gt[3]-last_gt[1]) + (detection_box[t,3] - detection_box[t,1]) * (detection_box[t,2] - detection_box[t,0]) - tmp))
            ...
            for ii in range(k):
                ...
                pixel_count[ii] = (x2-x1)*(y2-y1) / float((last_gt[2]-last_gt[0]) * (last_gt[3]-last_gt[1]) + (bb[3] - bb[1]) * (bb[2] - bb[0]) - (x2-x1)*(y2-y1))
