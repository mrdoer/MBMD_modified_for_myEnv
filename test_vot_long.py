...
from mdnet_utils import crop_image
...
def main(_):
    ...
    # initFeatOp: init_feature_maps
    # initInputOp: input_init_image
    ...
    researchImageOp = tf.placeholder(dtype=tf.float32, shape=(256, 107, 107, 3))
    researchOutputsOp = mdnet.vggM(researchImageOp,reuse=True)

    imageSingleOp = tf.placeholder(dtype = tf.float32, shape = (1,107,107,3))
    outputsSingleOp = mdnet.vggM(imageSingleOp,reuse=True) # Verification Op ---classification
    ...
    logitsOp,classOp = mdnet.classification(featInputOp)
    lossOp,lossOp1 = mdnet.loss(logitsOp, labelOp)
    ...
    featOp = mdnet.extractFeature(imageOp1) # VGGM features extraction Op
    ...
    for title_id in range(0,len(titles)):
        ...
        # extract template feature on the 1st frame
        ...
        num_iter = math.floor(5000 / 256)
        print('  Neg num_iter: {}'.format(num_iter))
        for t in range(num_iter):
            # extract VGGM features of neg_regions
            neg_features[t*256:(t+1)*256,:,:,:] = sess.run(featOp,feed_dict={imageOp1:neg_regions[t*256:(t+1)*256,:,:,:]})
        residual = 5000 - 256*num_iter
        tmp = math.floor(256 / residual) + 1
        ...
        num_iter = math.floor(500 / 256)
        print('  Pos num_iter: {}'.format(num_iter))
        for t in range(num_iter):
            # extract VGGM features of neg_regions
        ...
        search_box1 = [init_gt[1], init_gt[0], init_gt[3] - init_gt[1], init_gt[2] - init_gt[0]]
        search_box1 = np.reshape(search_box1, (1, 4))
        search_regions = extract_regions(np.array(init_img), search_box1)
        search_regions = search_regions[:,:,::-1]
        score_first = sess.run(outputsSingleOp, feed_dict={imageSingleOp: search_regions})
        #print score_first
        ...
        pos_regions_record = []
        neg_regions_record = []
        first_w = init_gt[3] - init_gt[1]
        first_h = init_gt[2] - init_gt[0]
        for i in range(1,num_frames):
            ...
            if (search_box1[1] <= 0 and search_box1[3] <= 0) or (search_box1[0] <= 0 and search_box1[2] <= 0) or (search_box1[1] >= cur_ori_img.width-1 and search_box1[3] >= cur_ori_img.width - 1) or (search_box1[0] >= cur_ori_img.height-1 and search_box1[2] >= cur_ori_img.height-1):
                score_max = -1
            else:
            ...
            if score_max < 0:
                gt_tmp = np.array([last_gt[1], last_gt[0], last_gt[3] - last_gt[1],
                                   last_gt[2] - last_gt[0]])
                candidates_samples = gen_samples(SampleGenerator('gaussian', cur_ori_img.size, 0.6, 1.05,valid=True), gt_tmp, 256)
                candidates_regions = extract_regions(cur_ori_img_array, candidates_samples)
                candidates_regions = candidates_regions[:, :, :, ::-1]
                researchScores = sess.run(researchOutputsOp, feed_dict={researchImageOp: candidates_regions})
                ...
                if score_max > 0:
                    detection_box = np.array([target_bbox[1],target_bbox[0],target_bbox[3]+target_bbox[1],target_bbox[2]+target_bbox[0]])
            ...
            if scores[0, max_idx] < 0.3: #and score_max < 20.0:
                ...
                if score_max < 0:
                    search_gt[0] = cur_ori_img.height / 2.0 - first_h / 2.0
                    search_gt[2] = cur_ori_img.height / 2.0 + first_h / 2.0
                    search_gt[1] = cur_ori_img.width / 2.0 - first_w / 2.0
                    search_gt[3] = cur_ori_img.width / 2.0 + first_w / 2.0
                ...
                if score_max < 0:
                    search_gt[0] = cur_ori_img.height / 2.0 - first_h / 2.0 /2.0
                    search_gt[2] = cur_ori_img.height / 2.0 + first_h / 2.0 /2.0
                    search_gt[1] = cur_ori_img.width / 2.0 - first_w / 2.0 /2.0
                    search_gt[3] = cur_ori_img.width / 2.0 + first_w / 2.0/2.0
            ...
            if scores[0,max_idx] > 0.7 and score_max > 0:
                ...
                if pos_feat_record.shape[0] > 500+1:
            ...
            if np.mod(i,10) == 0:
                for iter in range(30):
                    ...
                    if len(neg_feat_last) > 0:
                        neg_feat = np.random.randint(0, neg_feat_record.shape[0], 96-neg_feat_last.shape[0])
                    ...
                    featInputs = np.concatenate((pos_feat, neg_feat), axis=0)
                    _, loss1, logits1 = sess.run([trainVGGMOp, lossOp1, logitsOp],
                                                 feed_dict={featInputOp: featInputs, labelOp: labels, lrOp: 0.0002})
                    logits2 = sess.run(classOp,feed_dict={featInputOp:featInputs})

                    hard_neg = np.argsort(-logits2[32:,1])
                    neg_feat_last = featInputs[32:,:,:,:]
                    neg_feat_last = neg_feat_last[hard_neg[:15],:,:,:]
            ...
            show_res(cur_ori_img_array, np.array(last_gt, dtype=np.int32), '2', score=scores[0,max_idx],frame_id=i,all_frame=num_frames,score_max=score_max)
            # cv2.imshow("search_region",cur_img_array)
            # cv2.waitKey(1)
            bBoxes[i, :] = last_gt
